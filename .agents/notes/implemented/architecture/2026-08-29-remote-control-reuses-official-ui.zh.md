...
- **配对远程桌面的 host 模式。** 在 0.1.2-alpha.1 线上，host 侧 `/api` 面没有按方法的特权锁定——「配置面仅限本机」的行为在浏览器里，客户端插件按 `connection.isLoopback` 分支（设置镜像退化为 memory 作用域、文档控制器退化为内存桩）。因此通道 boot 脚本在一切 boot entry 之前安装传输钩子 `__DSH_TRANSPORT__ = { ownsHost: true }`，且仅当设备门控的 `/pair-app` 落地页在此之前发布了服务器签发的授权标记时（[一次性落地授权](2026-09-21-remote-one-time-landing-grant.zh.md)）：官方 UI 呈现完整配置面，所有调用仍走门控 `/remote` 通道以回环形态执行。这取代了最初设想的 dsh-LAN 式 `/lanapi` 代理——那是为 host 侧锁定特权方法的线设计的；我们不再维护任何端点表。
