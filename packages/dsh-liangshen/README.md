...
9. the `reasoning-effort` plugin joins the host's `agent/request` waterfall and swaps the request's reasoning level at a plan-mode boundary (deep while planning, shallow for single-step execution). It switches only at that boundary, because the field governs cache reuse and flipping it every turn would pay a cache miss per turn; and it never requests a level the routed model does not offer, so an unreadable or narrower level set leaves the frozen configuration standing.
...
| `planningEffort` / `executionEffort` | `high` / `low` | Reasoning levels the `reasoning-effort` plugin requests through the host's `agent/request` waterfall: a deep budget while plan mode is forming the work, a shallow one for single-step execution. The switch happens only at a plan-mode boundary, because the field governs cache reuse — flipping it every turn would pay a cache miss per turn. Levels are validated against the DeepSeek adapter's set (`off`/`low`/`high`/`max`) and the switch is skipped when the routed model's offered levels are readable and exclude the wanted one. |
