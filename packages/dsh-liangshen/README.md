...
5. tool paging holds matching tools (default pattern `mcp__*`) off the wire — and, under `ptc`, out of the scope's registry view, which is what the `tools:sdk` section renders and what a program may dispatch to — until activated: the model calls `tool_activate({ namespace })`, the namespace becomes reachable again from the next request, and at most three paged namespaces stay active at once — activating a fourth evicts the least recently used one back to its summary. Activation state is rebuilt from the durable session event stream, so compactions and resumes restore the same surface;
...
- **Persistent shell on both platforms**: the preset mounts the upstream persistent PTY stack on every host, mirroring the builtin Minimal preset. POSIX mounts the `bash` stack; win32 mounts its pwsh twin — the `@deepseek-ai/dsh-terminal-bash` backend with `shellDialect: pwsh` plus `@deepseek-ai/dsh-tool-pwsh-persistent`. Each platform exposes exactly one shell tool (`bash` on POSIX, `pwsh` on win32), and shell state survives across calls on both. The preset ships no shell implementation of its own.
...
| `presentation` | `ptc` | Wire presentation for the whole session. `ptc` collapses the wire to `run_code` with every other tool reached through the generated SDK — the cheapest static context, and the shipped default; `native` keeps the assembled native roster, which the official scaffold comparison ranks higher on both code-agent benchmarks; `both` keeps the full roster and adds `run_code` co-resident. `ptc` and `both` need a mounted code runtime and degrade to `native` with a one-time warning when it is missing. Under `ptc` paging is enforced at the registry as well, because a collapsed wire has nothing left to filter; under `native` and `both` it stays a wire filter, so `both` keeps its documented SDK escape hatch. |
| `pagedToolPatterns` | `['mcp__*']` | Glob patterns whose matching tools are held back from the wire until the model activates their namespace through `tool_activate({ namespace })`; held-back namespaces appear in the catalog as one-line summaries. Under `ptc` the same names leave the generated SDK, so the held-back namespace is unreachable from the FIRST request on — an unactivated namespace is neither listed in `tools:sdk` nor callable inside a `run_code` program. At most three paged namespaces stay active, and activation state is rebuilt from the durable event stream across compactions and resumes. Empty disables paging. |
...
- The persistent shell replaces the Standard ephemeral shell for the whole session, so shell state survives across calls: the persistent `bash` registers on POSIX, and on win32 the upstream persistent `pwsh` registers instead — each host mounts exactly one shell tool;
