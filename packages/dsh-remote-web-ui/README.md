...
- **Turning the pairing policy off does not open `/remote`**: the `/remote` prefix keeps proxying a stale same-origin rewrite, but the inner `dsh-auth-*` credential is attached only to a request that itself presented a live paired-device credential. An unpaired caller on a LAN or tunnel deployment is forwarded without that credential and the inner route answers 401, so `requirePairingForLan: false` never hands machine-owner access to a caller that did not pair.
- **The desktop gate policy is public**: `/api/pair/status` exposes the boolean `requirePairingForLan` policy plus the pairing phase and LAN availability, so a remote desktop can choose the correct transport before its settings scope is available. It is not a credential: an unpaired caller never receives the token, the device roster, counts, tunnel URLs, or the host's private LAN literals.
