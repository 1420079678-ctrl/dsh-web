...
- **The group is optional for the family plugins**: this package declares the `web-ui.plugin.item` list seat the family cards register into. A profile that installs a family plugin without this group carries only the official keyed `settings.plugin.item` seat of the Plugins tab, and each family plugin registers its card there instead (keyed by its settings namespace), so no card silently disappears.
